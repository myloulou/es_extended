function CreateExtendedPlayer(player, accounts, inventory, job, org, loadout, name, lastPosition)
  ...
  self.setOrg = function(name, gradeorg)
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        self.org['id']    = result[1].id
        self.org['name']  = result[1].name
        self.org['label'] = result[1].label
        ...
        MySQL.Async.fetchAll(
          ...
          function(result)
            ...
            self.org['gradeorg']        = gradeorg
            self.org['gradeorg_name']   = result[1].name
            self.org['gradeorg_label']  = result[1].label
            self.org['gradeorg_salary'] = result[1].salary

            self.org['skin_male']    = nil
            self.org['skin_female']  = nil

            if result[1].skin_male ~= nil then
              self.org['skin_male'] = json.decode(result[1].skin_male)
            end

            if result[1].skin_female ~= nil then
              self.org['skin_female'] = json.decode(result[1].skin_female)
            end
